...
class SWFMatrix {
	...
	fun parse(data: SWFData) {
		...
		val px = matrix.deltaTransformPoint(MPoint(0.0, 1.0))
...
class SWFRectangle(
	...
	protected var _rectangle = MRectangle()
...
open class SWFShape(var unitDivisor: Double = 20.0) {
	...
	private var coordMap = HashMap<MPoint, FastArrayList<IEdge>>()
        ...
        for (i in 0 until records.size) {
            ...
            when (shapeRecord.type) {
                ...
                SWFShapeRecord.TYPE_STRAIGHTEDGE -> {
                    ...
                    from = MPoint(NumberUtils.roundPixels400(xPos), NumberUtils.roundPixels400(yPos))
                    ...
                    to = MPoint(NumberUtils.roundPixels400(xPos), NumberUtils.roundPixels400(yPos))
                ...
                SWFShapeRecord.TYPE_CURVEDEDGE -> {
                    ...
                    control = MPoint(xPosControl, yPosControl)
                    to = MPoint(NumberUtils.roundPixels400(xPos), NumberUtils.roundPixels400(yPos))
	...
	protected fun exportFillPath(handler: ShapeExporter, groupIndex: Int) {
		...
		var pos = MPoint(Int.MAX_VALUE, Int.MAX_VALUE)
		...
		if (path.size > 0) {
			...
			for (i in 0 until path.size) {
				...
				if (fillStyleIdx != e.fillStyleIdx) {
					...
					pos = MPoint(Int.MAX_VALUE, Int.MAX_VALUE)
					try {
						var matrix: MMatrix
