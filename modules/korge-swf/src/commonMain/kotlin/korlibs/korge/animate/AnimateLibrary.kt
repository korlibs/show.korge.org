...
class AnTextFieldSymbol(id: Int, name: String?, val initialHtml: String, val bounds: MRectangle) : AnSymbol(id, name) {
	override fun create(library: AnLibrary): AnElement = AnTextField(library, this)
}

open class AnSymbolBaseShape(id: Int, name: String?, var bounds: MRectangle, val path: VectorPath? = null) :
	AnSymbol(id, name) {
}
...
// @TODO: MMatrix and ColorTransform pools? Maybe smaller size, reusability and possibility of compute values fast
data class AnSymbolTimelineFrame(
	...
	fun setToInterpolated(l: AnSymbolTimelineFrame, r: AnSymbolTimelineFrame, ratio: Ratio) {
