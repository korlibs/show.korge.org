...
import korlibs.io.stream.writeF32LE
import korlibs.io.stream.writeS_VL
...
object AnLibrarySerializer {
	...
	private fun SyncStream.writeRect(r: MRectangle) {
	...
	private fun SyncStream.writeIRect(r: RectangleInt) {
		writeS_VL(r.x)
		writeS_VL(r.y)
		writeS_VL(r.width)
		writeS_VL(r.height)
	...
	suspend private fun SyncStream.writeLibrary(lib: AnLibrary, config: Config, externalWriters: ExternalWriters) {
		...
		val atlasBitmaps = listOf(
			lib.symbolsById.filterIsInstance<AnSymbolShape>().map { it.textureWithBitmap?.bitmapSlice?.base },
			lib.symbolsById.filterIsInstance<AnSymbolMorphShape>().flatMap { it.texturesWithBitmap.entries.map { it.second.bitmapSlice.base } }
		...
		for (symbol in lib.symbolsById) {
			...
			when (symbol) {
				...
				is AnSymbolShape -> {
					...
					writeU_VL(atlasBitmapsToId[symbol.textureWithBitmap!!.bitmapSlice.base]!!)
				...
				is AnSymbolMorphShape -> {
					...
					for ((ratio1000, textureWithBitmap) in entries) {
						...
						writeU_VL(atlasBitmapsToId[textureWithBitmap.bitmapSlice.base]!!)
						writeRect(textureWithBitmap.bounds.mutable)
