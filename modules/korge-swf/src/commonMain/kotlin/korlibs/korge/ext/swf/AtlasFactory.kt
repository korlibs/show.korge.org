...
data class BitmapWithScale(val bitmap: Bitmap, val scale: Double, val bounds: MRectangle) : Extra by Extra.Mixin() {
        ...
        for (at in atlas.atlases) {
            ...
            for (item in at.packedItems) {
                ...
                out[item.item.key] = TextureWithBitmapSlice(
                    texture = texture.slice(rect2.toInt()),
                    bitmapSlice = item.slice,
                    scale = ibmp.scale,
                    bounds = ibmp.bounds.immutable
                )
            }
        }
        return out
    } else {
        return this.entries.associate { it.key to TextureWithBitmapSlice(it.value.bitmap.slice(), it.value.bitmap.slice(), it.value.scale, it.value.bounds.immutable) }
    }
    //val borderSize = 4
    /*
    val borderSize = 8
