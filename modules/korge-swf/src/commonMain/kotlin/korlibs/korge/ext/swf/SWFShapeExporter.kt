...
class SWFShapeExporter(
	...
	//val bounds: MRectangle = dshape.shapeBounds.rect
...
open class SWFBaseShapeExporter(
    ...
    override fun beginBitmapFill(bitmapId: Int, matrix: MMatrix, repeat: Boolean, smooth: Boolean) {
        flush()
        drawingFill = true
        val bmp = bitmaps[bitmapId] ?: Bitmap32(1, 1)
        fillStyle = BitmapPaint(
            bmp, matrix.immutable, CycleMethod.fromRepeat(repeat), CycleMethod.fromRepeat(repeat), smooth
        )
    }
    ...
    private fun __flushFill() {
        ...
        shapes += FillShape(apath, null, fillStyle, Matrix.IDENTITY)
        apath = VectorPath()
    }
